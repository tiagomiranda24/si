...
class Dropout(Layer):
    ...
    def forward_propagation(self, input: np.ndarray, training: bool) -> np.ndarray:
        ...
        if training:
            ...
            # Compute the mask using a binomial distribution
            ...
            # In inference mode, return the input unmodified
            self.output = input
        return self.output
    ...
    def output_shape(self) -> tuple:
        """
        Return the shape of the input (Dropout does not change the shape of the data).
    ...
    def parameters(self) -> int:
        """
        Dropout layers do not have learnable parameters.
